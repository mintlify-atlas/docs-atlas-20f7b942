---
title: "LangGraph Orchestration: Stateful Subgraphs and Agentic Loops"
sidebarTitle: "LangGraph"
description: "Learn how LangGraph orchestrates the agentic pipeline with stateful subgraphs, conditional loop edges, and fan-out/fan-in patterns for parallel retrieval."
---
...
- **Observable** — LangGraph's graph execution trace records every node entry, state transition, and edge traversal. This makes it straightforward to debug multi-hop chains, inspect intermediate sub-answers, and understand exactly which retrieval hops contributed to the final output.

## Graph Structure

The pipeline is organised as a three-level hierarchy: a parent graph that owns the top-level question decomposition and synthesis, and two parallel subgraphs — one for each retrieval channel.

### Parent Graph

The parent graph coordinates the full pipeline from user query to final answer:

| Step | Node | Role |
|---|---|---|
| 1 | **Query intake** | Accepts the raw user question |
| 2 | **Decomposition** | Breaks the question into semantic sub-queries (with `#N` back-references) and relational SPO triple queries (with `Entity#N` placeholders) |
| 3 | **Fan-out** | Dispatches both the semantic channel subgraph and the relational channel subgraph in parallel |
| 4 | **Fan-in / merge** | Waits for both subgraphs to complete and merges their accumulated sub-answers |
| 5 | **Synthesis** | Produces the final answer by combining the merged evidence from both channels |
...
| **Triplet summaries** | Summarises the filtered triplets into a structured relational context |
