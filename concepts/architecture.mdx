...
The relational channel retrieves and reasons over structured entity/relationship triples from the Neo4j knowledge graph. Its pipeline is more direct than the semantic channel, focusing on graph traversal rather than text summarisation:

<Steps>
  <Step title="SPO Triple Queries">
    The clinical question is decomposed into Subject–Predicate–Object triple queries, with `Entity#N` placeholders that can reference entities resolved in earlier hops. These structured queries are issued to the Neo4j knowledge graph.
  </Step>
